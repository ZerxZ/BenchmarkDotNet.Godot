...
namespace BenchmarkDotNet.Godot.Toolchains.InGodotProcess.NoEmit;
 ...
 public class InGodotProcessValidator : IValidator
    {
        ...
        private static string ValidateToolchain(Job job, Characteristic characteristic) =>
            ...
            || job.Infrastructure.Toolchain is InProcessNoEmitToolchain || job.Infrastructure.Toolchain is InGodotProcessNoEmitToolchain
                ? null
        ...
        public static readonly IValidator DontFailOnError = new InGodotProcessValidator(false);

        /// <summary>The instance of validator that DOES fail on error.</summary>
        public static readonly IValidator FailOnError = new InGodotProcessValidator(true);
        ...
        private InGodotProcessValidator(bool failOnErrors)
        {
        ...
        public IEnumerable<ValidationError> Validate(ValidationParameters validationParameters)
        {
            ...
            foreach (var benchmarkWithArguments in validationParameters.Benchmarks.Where(benchmark => benchmark.HasArguments && GetToolchain?.Invoke(null, new[] { benchmark }) is InProcessNoEmitToolchain))
        ...
        private MethodInfo? GetToolchain = Type.GetType("BenchmarkDotNet.Toolchains.ToolchainExtensions")?.GetMethod("GetToolchain", BindingFlags.Static | BindingFlags.NonPublic, null, new Type[] { typeof(BenchmarkCase) }, null);
    }