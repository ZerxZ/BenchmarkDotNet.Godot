...
public class InGodotProcessNoEmitRunner
{
    public static int Run(IHost host, BenchmarkCase benchmarkCase)
    {
        ...
        try
        {
            ...
            string inProcessRunnableTypeName = $"{typeof(InGodotProcessNoEmitRunner).FullName}+{nameof(Runnable)}";
            var type = typeof(InGodotProcessNoEmitRunner).GetTypeInfo().Assembly.GetType(inProcessRunnableTypeName)
